...
import { Dimensions, Keyboard, TextInput, ScrollView, Platform, NativeModules, LayoutAnimation } from 'react-native';
...
class RenderBackToHome extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loadButton: false
  ...
  render() {
    const { submitButtonStyle, refershChat } = this.props;
    return (
      <React.Fragment>
...
class ChatBot extends Component {
  ...
  triggerNextStep(data) {
    ...
    if (isEnd) {
      this.handleEnd();
    } else if ((currentStep.options && data) || (data && data.customOption)) {
      let option = null;
  ...
  render() {
    ...
    const { user, end, metadata } = currentStep;
    return (
      <ChatBotContainer
      ...
      >
        ...
        >
          ...
          {end && <RenderBackToHome
            submitButtonStyle={submitButtonStyle}
            refershChat={this.props.refershChat}
          />
          }
        </ScrollView>

        {(user === true && (metadata && metadata.isUser === true)) && <InputView
